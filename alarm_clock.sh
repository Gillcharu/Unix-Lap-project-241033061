...
beep() { for i in {1..5}; do printf "\a"; sleep 0.2; done; }
...
time_diff() {
    ...
    now=$((curr_h*60 + curr_m)); target=$((target_h*60 + target_m))
...
add_snooze() {
    ...
    m=$((m + snooze))
    if (( m >= 60 )); then h=$((h + m/60)); m=$((m % 60)); fi
    (( h >= 24 )) && h=$((h % 24))
    printf "%02d:%02d" "$h" "$m"
}
...
ring_alarm() {
    ...
    play_sound
    for i in {1..3}; do echo -e "${RED}${BOLD}>>> WAKE UP! <<<${NC}"; sleep 1; done
...
while true; do
    ...
    1)
        ...
        1)
            ...
            if ! validate_time "$alarm_time"; then
                echo -e "${RED}Invalid time format.${NC}"; sleep 2; continue
            fi
            ;;
        2)
            read -rp "Enter minutes from now: " mins
            if ! [[ "$mins" =~ ^[0-9]+$ ]]; then echo -e "${RED}Invalid number.${NC}"; sleep 2; continue; fi
            ...
            ;;
        *)
            echo -e "${RED}Invalid mode.${NC}"; sleep 2; continue
            ;;
        esac

        alarm_hour=${alarm_time%:*}; alarm_min=${alarm_time#*:}
        ...
        while true; do
            ...
            if (( curr_h == alarm_hour && curr_m == alarm_min )); then
                ...
                if (( $? == 1 )); then
                    ...
                    alarm_hour=${new_time%:*}; alarm_min=${new_time#*:}; alarm_time="$new_time"
                    sleep 60
                else break
                fi
            fi
    ...
    2)
        ...
        ;;
    3) view_alarms; read -rp "Press Enter to continue..." _ ;;
    4) echo -e "${GREEN}Exiting Alarm Clock. Goodbye!${NC}"; exit 0 ;;
